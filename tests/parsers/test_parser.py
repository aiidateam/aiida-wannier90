...
def test_bvectors_not_enough(
    ...
    assert (
        ...
        == Wannier90Calculation.exit_codes.ERROR_BVECTORS.status
    )
...
def test_disentanglement_not_enough_states(
    ...
    assert (
        calcfunction.exit_status
        == Wannier90Calculation.exit_codes.ERROR_DISENTANGLEMENT_NOT_ENOUGH_STATES.status
    )


def test_plot_wf_cube(fixture_localhost, generate_calc_job_node, generate_parser):
    ...
    assert (
        calcfunction.exit_status
        == Wannier90Calculation.exit_codes.ERROR_PLOT_WF_CUBE.status
    )
...
def test_output_stdout_incomplete(
    ...
    assert (
        calcfunction.exit_status
        == Wannier90Calculation.exit_codes.ERROR_OUTPUT_STDOUT_INCOMPLETE.status
    )
